...
The REST API documentation can be found [on docs.midday.com](https://docs.midday.com). The full API of this library can be found in [api.md](api.md).
...
async function main() {
  const account = await midday.accounts.list();
...
async function main() {
  const account: Midday.Account = await midday.accounts.list();
}
...
```ts
async function main() {
  const account = await midday.accounts.list().catch(async (err) => {
...
const response = await midday.accounts.list().asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: account, response: raw } = await midday.accounts.list().withResponse();
