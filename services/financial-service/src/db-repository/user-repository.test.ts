...
describe("UserRepository Tests", () => {
  ...
  describe("Read Operations", () => {
    ...
    it("should search users by name or email", async () => {
      const results = await repository.findByNameOrEmail(
        testCtx.user.name || "",
      );
  ...
  describe("Delete Operations", () => {
    it("should delete user and associated API keys", async () => {
      ...
      expect(
        Object.values(deleteTestCtx.apiKeys).filter((k) => k !== null).length,
      ).toBeGreaterThan(0);
  ...
  describe("Dependent Records", () => {
    ...
    it("should verify all API keys are deleted with user", async () => {
      ...
      const initialApiKeys = Object.values(verifyTestCtx.apiKeys).filter(
        (k) => k !== null,
      ).length;
      ...
      const remainingKeys = await apiKeyRepository.getByUserId(
        verifyTestCtx.user.id,
      );
