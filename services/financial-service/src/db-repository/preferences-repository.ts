...
export class PreferencesRepository {
    ...
    private validatePreferences(prefs: Partial<Preferences>, currentPrefs?: Preferences) {
        ...
        if ((prefs.serviceTier === 'enterprise' || currentTier === 'enterprise') &&
    ...
    private prepareForDatabase(prefs: Partial<Preferences>): Partial<Preferences> {
        ...
        if (prepared.webhookEvents !== undefined) {
            prepared.webhookEvents = prepared.webhookEvents === null ? null :
                prepared.webhookEvents as WebhookEventType[];
        }
        if (prepared.ipWhitelist !== undefined) {
            prepared.ipWhitelist = prepared.ipWhitelist === null ? null :
    ...
    async create(prefs: NewPreferences): Promise<Preferences> {
        ...
        const [createdPreferences] = await this.db.insert(preferences)
            .values({
                ...preparedData,
                userId: prefs.userId,
                createdAt: new Date(),
    ...
    async getById(id: number): Promise<Preferences | null> {
        const [prefs] = await this.db.select()
            .from(preferences)
    ...
    async update(id: number, prefs: Partial<Preferences>): Promise<Preferences> {
        ...
        const [updatedPreferences] = await this.db.update(preferences)
            .set({
                ...preparedData,
                updatedAt: new Date()
            })
    ...
    async delete(id: number): Promise<boolean> {
        const result = await this.db.delete(preferences)
            .where(eq(preferences.id, id))
            .returning();
    ...
    async getUsersByServiceTier(tier: ServiceTier): Promise<number[]> {
        const results = await this.db.select({ userId: preferences.userId })
            .from(preferences)
    ...
    private isValidPreferences(preferences: Partial<Preferences>): preferences is Preferences {
        const requiredKeys: (keyof Preferences)[] = [
            'id', 'createdAt', 'updatedAt', 'userId', 'serviceTier', 'complianceLevel'
        ];
