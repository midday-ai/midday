...
describe("APIKeyRepository Integration Tests", () => {
  ...
  describe("Basic CRUD Operations", () => {
    ...
    describe("read operations", () => {
      ...
      it("should retrieve all API keys for a user", async () => {
        ...
        expect(
          results.some((key) => key.id === testCtx.apiKeys.created!.id),
        ).toBeTruthy();
      });
    });
      ...
      try {
        await db.executeTransaction(async (tx) => {
          ...
          await repository.create(
            APIKeyGenerator.generate({ userId: testCtx.user.id }),
          );
  ...
  describe("Error Cases", () => {
    it("should handle invalid scope values", async () => {
      ...
      await expect(repository.create(input)).rejects.toThrow(
        "Validation failed: Invalid scope array",
      );
    });
    ...
    it("should handle invalid IP addresses", async () => {
      ...
      await expect(repository.create(input)).rejects.toThrow(
        "Validation failed: Invalid IP addresses array",
      );
    });
  ...
  describe("Business Logic Tests", () => {
    describe("getActiveKeysByUserId", () => {
      it("should only return active and non-revoked keys", async () => {
        ...
        expect(
          results.some((key) => key.id === testCtx.apiKeys.active!.id),
        ).toBeTruthy();
        expect(
          results.some((key) => key.id === testCtx.apiKeys.revoked!.id),
        ).toBeFalsy();
      });
    });
    ...
    describe("getExpiringKeys", () => {
      ...
      it("should not include already expired keys", async () => {
        const results = await repository.getExpiringKeys(7);
        const expiredKey = results.find(
          (k) => k.id === testCtx.apiKeys.expired!.id,
        );
    ...
    describe("getHighUsageKeys", () => {
      it("should identify keys with high usage", async () => {
        ...
        const foundKey = results.find(
          (key) => key.id === testCtx.apiKeys.highUsage!.id,
        );
        expect(foundKey).toBeDefined();
      });
    });
