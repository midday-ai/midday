...
export class APIKeyGenerator {
  ...
  public static generate(
    ...
    const expiresAt =
      options.expiresAt ||
      new Date(
        now.getTime() + 365 * 24 * 60 * 60 * 1000 + this.counter++ * 60 * 1000,
      );
    ...
    return {
      ...
      environment:
        `${options.environment || "development"}` as APIKeyEnvironment,
  ...
  public static generateExpired(
    options: APIKeyGeneratorOptions = {},
  ): Omit<APIKey, "id" | "createdAt"> {
    const now = new Date();
    // Generate a unique past date for expiration
    const pastDate = new Date(
      now.getTime() - 24 * 60 * 60 * 1000 - this.counter++ * 60 * 1000,
    );
  ...
  public static generateHighUsage(
    options: APIKeyGeneratorOptions = {},
  ): Omit<APIKey, "id" | "createdAt"> {
    return this.generate({
      ...options,
