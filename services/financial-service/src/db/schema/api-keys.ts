...
export function isValidScope(scope: unknown): scope is APIKeyScope {
  ...
  return (
    typeof scope === "string" && validScopes.includes(scope as APIKeyScope)
  );
}

// Helper function to validate environment
export function isValidEnvironment(env: unknown): env is APIKeyEnvironment {
  const validEnvironments: APIKeyEnvironment[] = [
    "development",
    "staging",
    "production",
  ];
  return (
    typeof env === "string" &&
    validEnvironments.includes(env as APIKeyEnvironment)
  );
}
...
export function validateNewAPIKey(key: NewAPIKey): string[] {
  ...
  if (
    key.scope &&
    (!Array.isArray(key.scope) || !key.scope.every(isValidScope))
  ) {
  ...
  if (
    key.allowedIPs &&
    (!Array.isArray(key.allowedIPs) || !key.allowedIPs.every(isValidIPAddress))
  ) {
  ...
  if (
    key.allowedDomains &&
    (!Array.isArray(key.allowedDomains) ||
      !key.allowedDomains.every(isValidDomain))
  ) {
  ...
  if (
    key.rateLimit !== undefined &&
    (!Number.isInteger(key.rateLimit) || key.rateLimit < 0)
  ) {
  ...
  if (
    ...
    (!Number.isInteger(key.usageCount) || key.usageCount < 0)
  ) {
