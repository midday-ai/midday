...
export const preferences = sqliteTable("user_preferences", {
  ...
  serviceTier: text("service_tier")
    .$type<ServiceTier>()
    .notNull()
    .default("starter"), // Fintech tiers: starter, pro, enterprise
  complianceLevel: text("compliance_level")
    .$type<ComplianceLevel>()
    .notNull()
    .default("basic"), // Compliance levels: basic, intermediate, advanced
  ...
  webhookEvents: blob("webhook_events", { mode: "json" }).$type<
    WebhookEventType[]
  >(), // Specific events they want notifications for
  sandboxMode: integer("sandbox_mode", { mode: "boolean" }).default(true), // Enables testing environment
  testDataEnabled: integer("test_data_enabled", { mode: "boolean" }).default(
    true,
  ), // Allows use of test data for development and simulations
  ...
  emailNotificationApiKey: integer("email_notification_api_key", {
    mode: "boolean",
  }).default(true), // Whether API key notifications are enabled
  emailNotificationQuota: integer("email_notification_quota", {
    mode: "boolean",
  }).default(true), // Quota for notification-related emails
  emailNotificationSecurity: integer("email_notification_security", {
    mode: "boolean",
  }).default(true), // Security alerts for user accounts
  emailNotificationCompliance: integer("email_notification_compliance", {
    mode: "boolean",
  }).default(true), // Compliance alerts, such as failed KYC
  transactionAlertThreshold: integer("transaction_alert_threshold").default(
    10000,
  ), // Threshold to trigger transaction alerts
  transactionAlertEnabled: integer("transaction_alert_enabled", {
    mode: "boolean",
  }).default(true), // Controls whether alerts are enabled
  ...
  createdAt: integer("created_at", { mode: "timestamp" })
    .notNull()
    .default(sql`CURRENT_TIMESTAMP`), // Timestamp when preferences were created
...
export function isValidComplianceLevel(
  level: unknown,
): level is ComplianceLevel {
  const validLevels: ComplianceLevel[] = ["basic", "standard", "enhanced"];
  return (
    typeof level === "string" && validLevels.includes(level as ComplianceLevel)
  );
}
...
export function isValidWebhookEvent(event: unknown): event is WebhookEventType {
  ...
  return (
    typeof event === "string" && validEvents.includes(event as WebhookEventType)
  );
}
...
export function validateRateLimits(
  ...
  if (
    ...
    !isValidPositiveInteger(maxRequestsPerSecond)
  ) {
  ...
  if (
    ...
    !isValidPositiveInteger(maxTransactionsPerDay)
  ) {
  ...
  if (
    ...
    !isValidPositiveInteger(maxAccountsPerUser)
  ) {
...
export function validatePreferences(
  prefs: Partial<NewPreferences> | Partial<Preferences>,
): string[] {
  const errors: string[] = [];
  ...
  if (
    "complianceLevel" in prefs &&
    !isValidComplianceLevel(prefs.complianceLevel)
  ) {
  ...
  if (
    "emailNotificationApiKey" in prefs &&
    !isValidBoolean(prefs.emailNotificationApiKey)
  ) {
    errors.push("emailNotificationApiKey must be a boolean value");
  }

  if (
    "emailNotificationQuota" in prefs &&
    !isValidBoolean(prefs.emailNotificationQuota)
  ) {
    errors.push("emailNotificationQuota must be a boolean value");
  }

  if (
    "emailNotificationSecurity" in prefs &&
    !isValidBoolean(prefs.emailNotificationSecurity)
  ) {
    errors.push("emailNotificationSecurity must be a boolean value");
  }

  if (
    "emailNotificationCompliance" in prefs &&
    !isValidBoolean(prefs.emailNotificationCompliance)
  ) {
  ...
  if (
    prefs.auditLogRetention !== undefined &&
    !isValidPositiveInteger(prefs.auditLogRetention)
  ) {
  ...
  if (
    prefs.transactionAlertThreshold !== undefined &&
    !isValidPositiveInteger(prefs.transactionAlertThreshold)
  ) {
    errors.push("transactionAlertThreshold must be a non-negative integer");
  }

  if (
    "transactionAlertEnabled" in prefs &&
    !isValidBoolean(prefs.transactionAlertEnabled)
  ) {
...
export function validateServiceTierLimits(
  ...
  switch (tier) {
    ...
    case "enterprise":
      ...
      if (prefs.maxTransactionsPerDay && prefs.maxTransactionsPerDay > 100000) {
        errors.push(
          "Enterprise tier cannot exceed 100000 transactions per day",
        );
      }
...
export function createDefaultPreferences(
  serviceTier: ServiceTier = "starter",
): Omit<NewPreferences, "userId"> {
