...
async function migrateExistingTeamCategories(db: Database, teamId: string) {
    ...
    for (const child of parent.children) {
      if (!existingSlugs.has(child.slug)) {
        ...
        if (existing) {
          ...
          if (
            existing.taxRate !== getTaxRateForCategory(countryCode, child.slug)
          ) {
  ...
  if (categoriesToUpdate.length > 0) {
    ...
    console.log(`✅ Updated ${categoriesToUpdate.length} existing categories`);
  }
  ...
  console.log(`🔗 Updated ${parentChildUpdates} parent-child relationships`);
