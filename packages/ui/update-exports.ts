...
function generateExports(dir: string, baseDir: string = ''): Record<string, string> {
    ...
    for (const file of files) {
        ...
        } else if ((file.endsWith('.tsx') || file.endsWith('.ts')) && !file.endsWith('.stories.tsx')) {
            const relativePath = path.join(baseDir, file);
            let exportKey = relativePath.replace(/\.(tsx?|js)$/, '').replace(/\/index$/, '');
            exportKey = exportKey.replace(/^components\//, ''); // Remove "components/" prefix
...
function updatePackageJson(newExports: Record<string, string>) {
    ...
    // Remove leading "./" from export keys and "components/" prefix
