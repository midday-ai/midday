...
export interface NetExpenseChartProps extends React.HTMLAttributes<HTMLDivElement>, Omit<BarChartProps, "data"> {
    className?: string;
    title: string;
    viewMoreHref?: string;
    price: number;
...
export const NetExpenseChart: React.FC<NetExpenseChartProps> = ({ className, title, disabled, viewMoreHref, enableAssistantMode, enableComparison, price, priceChange, expenseMetrics, ...rest }) => {
    const rootClassName = cn("w-full max-w-4xl bg-background text-foreground p-6", className, disabled && "opacity-50 pointer-events-none");
    ...
    const netExpenseData = useMemo(() => {
        return FinancialExpenseAndIncomeMetricsConverter.convertDataToChartDataPoints(expenseMetrics, "expense");
    }, [expenseMetrics]); 0


    const yearlyTotalExpense = useMemo(() => {
        return FinancialExpenseAndIncomeMetricsConverter.computeTotalExpenseByYear(expenseMetrics);
    }, [expenseMetrics]);

    const yearlyAverageMonthlyExpense = useMemo(() => {
        return FinancialExpenseAndIncomeMetricsConverter.computeAverageMonthlyExpenseByYear(expenseMetrics);
    }, [expenseMetrics]);
    ...
    const monthlyExpense = useMemo(() => {
        return FinancialExpenseAndIncomeMetricsConverter.computeMonthlyExpense(expenseMetrics);
    }, [expenseMetrics]);
                    ...
                    {hasData ? (
                        <>
                            ...
                            <div className="flex items-center mt-1">
                                ...
                                <span className="ml-2 text-sm text-muted-foreground">vs {price} in previous month</span>
                            </div>
                        </>
                    ) : (
                ...
                {hasData && (
                    <Sheet>
                        ...
                        <SheetContent className="w-full md:min-w-[70%] scrollbar-hide overflow-y-auto">
                            <h2 className="text-lg font-semibold mb-4">Net Expense details</h2>
                            <h3 className="text-md font-semibold mt-4 mb-2">Expense this month</h3>
                            <div>
                                <BarChart
                                    currency="USD"
                                    data={netExpenseData}
                                ...
                                <Table>
                                    ...
                                    <TableBody>
                                        ...
                                        <TableRow>
                                            ...
                                            {years.map(year => (
                                                <TableCell key={year}>
                                                    ${yearlyTotalExpense[Number(year)]?.toFixed(2) || 'N/A'}
                                                </TableCell>
                                            ))}
                                        </TableRow>
                                    </TableBody>
                            ...
                            <div className="mt-6 border-t flex flex-col gap-6">
                                <div className="mt-6">
                                    <CategoryChart data={expenseByCategory} title={"Expense by Category"} description={"Expense by Category over time"} />
                                </div>
                                <Card className="flex flex-col gap-3 p-[2%] border-none">
                                    {monthlyExpense.map(({ month, year, totalExpense }) => (
                                        <div key={`${month}-${year}`}>
                                            <div className="flex flex-1 items-center justify-between" >
                                                <p className="text-md font-bold">{month} {year}</p>
                                                <p className="text-sm font-semibold">${totalExpense.toFixed(2)}</p>
                                            </div>
                                            <div className="flex flex-1 items-center justify-between">
                                                <p className="text-sm font-normal">{month.substring(0, 3)}'{year.toString().substring(2, 4)} Total Expense</p>
                                                <p className="text-xs font-semibold">{totalExpense.toFixed(2)}</p>
                                            </div>
                                        </div>
                                    ))}
                                </Card>
