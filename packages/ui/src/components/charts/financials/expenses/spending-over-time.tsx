...
import { FinancialExpenseAndIncomeMetricsConverter, FinancialMetricsTransactionConverter } from "../../../../lib/converters/expense-and-income-metrics-converter";
...
export interface SpendingOverTimeProps extends React.HTMLAttributes<HTMLDivElement> {
    className?: string;
    title: string;
    viewMoreHref?: string;
    price: number;
...
export const SpendingOverTime: React.FC<SpendingOverTimeProps> = ({ className, title, disabled, viewMoreHref, price, priceChange, expenseMetrics, budgetedAmount, transactions, ...rest }) => {
    ...
    const monthlyExpense = FinancialExpenseAndIncomeMetricsConverter.calculateMonthlyTotals(expenseMetrics, "expense");
    ...
    return (
        <Card className="bg-[#0b1727] text-white p-6 rounded-lg">
            <div className="flex justify-between items-center mb-4">
                ...
                <Sheet>
                    ...
                    <SheetContent className="w-full md:min-w-[80%] overflow-y-auto z-50 py-5 md:py-15">
                        <h2 className="text-xl font-semibold mb-4">Transactions by Month</h2>
                            ...
                            Object.entries(transactionsByMonth).map(([month, monthTransactions]) => (
                                <div key={month} className="mb-8 overflow-x-auto">
                                    <div className="flex flex-1 justify-between p-[2%]">
                                        ...
                                        <p className="text-xl font-base">${monthTransactions.reduce((acc, transaction) => acc + (transaction.amount || 0), 0).toFixed(2)}</p>
                                    </div>
                                            ...
                                            <TableBody>
                                                {monthTransactions.map((transaction, index) => (
                                                    <TableRow key={index} className="rounded-2xl">
                                                        ...
                                                        <TableCell>
                                                            <Badge variant="outline" className="p-2">{transaction.personalFinanceCategoryPrimary}</Badge>
                                                        </TableCell>
                                                    </TableRow>
                                                ))}
                                            </TableBody>
            ...
            <div className="text-center mb-4">
                <div className="text-2xl font-bold">${currentMonthExpense?.total} Spent This Month</div>
                {budgetedAmount && (
                    <div className="text-sm text-[#4a90e2]">out of ${budgetedAmount} budgeted</div>
                )}
            </div>
