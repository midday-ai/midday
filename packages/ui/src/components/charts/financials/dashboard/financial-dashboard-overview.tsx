...
export const MonthlySpendingChart: React.FC<MonthlySpendingChartProps> = ({ className, title, disabled, transactions: propTransactions, expenseMetrics: propExpenseMetrics, incomeMetrics: propIncomeMetrics, ...rest }) => {
    ...
    return (
        <div {...rest} className={rootClassName}>
            ...
            <div className="flex gap-3 items-center mb-2">
                <h3 className="text-lg font-semibold">Spending Across Top Categories</h3>
                <span className="text-md md:text-3xl font-bold">
                    ${topExpenseCategories.reduce((sum, category) => sum + parseFloat(category.total), 0).toFixed(2)}
                </span>
            </div>
                ...
                <div className="flex space-x-2 p-2 w-max">
                    {topExpenseCategories.map((category, index) => (
                        <div key={index} className="flex-shrink-0">
                            <Button variant="secondary" className="text-md flex items-center whitespace-nowrap">
                                <span className="text-md font-bold mr-3">{category.name}</span>
                                <span className="text-xs font-bold bg-secondary-foreground/10 px-2 py-1 rounded">${category.total}</span>
                            </Button>
                        </div>
                    ))}
                </div>
