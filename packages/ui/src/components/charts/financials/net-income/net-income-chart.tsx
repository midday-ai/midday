...
export interface NetIncomeChartProps extends React.HTMLAttributes<HTMLDivElement>, Omit<BarChartProps, "data"> {
    className?: string;
    title: string;
    viewMoreHref?: string;
    price: number;
...
export const NetIncomeChart: React.FC<NetIncomeChartProps> = ({ className, title, disabled, viewMoreHref, enableAssistantMode, enableComparison, price, priceChange, incomeMetrics, ...rest }) => {
    ...
    const netIncomeData = useMemo(() => {
        return FinancialExpenseAndIncomeMetricsConverter.convertDataToChartDataPoints(incomeMetrics, "income");
    }, [incomeMetrics]);

    console.log(netIncomeData);

    const yearlyTotalIncome = useMemo(() => {
        return FinancialExpenseAndIncomeMetricsConverter.computeTotalIncomeByYear(incomeMetrics);
    }, [incomeMetrics]);

    const yearlyAverageMonthlyIncome = useMemo(() => {
        return FinancialExpenseAndIncomeMetricsConverter.computeAverageMonthlyIncomeByYear(incomeMetrics);
    }, [incomeMetrics]);
    ...
    const monthlyIncome = useMemo(() => {
        return FinancialExpenseAndIncomeMetricsConverter.computeMonthlyIncome(incomeMetrics);
    }, [incomeMetrics]);
                ...
                {hasData && (
                    <Sheet>
                        ...
                        <SheetContent className="w-full md:min-w-[70%] scrollbar-hide overflow-y-auto">
                            ...
                            <h3 className="text-md font-semibold mt-4 mb-2">Income this month</h3>
                            <div>
                                <BarChart
                                    currency="USD"
                                    data={netIncomeData}
                            ...
                            <div className="mt-6 border-t flex flex-col gap-6">
                                <div className="mt-6">
                                    <CategoryChart data={incomeByCategory} title={"Income by Category"} description={"Income by Category over time"} />
                                </div>
                                <div className="flex flex-col gap-3">
                                    {monthlyIncome.map(({ month, year, totalIncome }) => (
                                        <div key={`${month}-${year}`}>
                                            ...
                                            <div className="flex flex-1 items-center justify-between">
                                                <p className="text-sm font-normal">{month.substring(0, 3)}'{year.toString().substring(2, 4)} Total Income</p>
                                                <p className="text-xs font-semibold">{totalIncome.toFixed(2)}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
