...
/**
 * Fetch exchange rates from the currency API for all supported currencies.
 * @returns Array of exchange rates with source currency, date, and rate mappings
 */
export async function getRates(): Promise<ExchangeRate[]> {
  ...
  return rates
    ...
    .filter((item): item is ExchangeRate => item !== null);
}
