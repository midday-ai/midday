...
async function generateTransactionsPDF(
    ...
    const addText = (text: string, x: number, y: number, options: any = {}) => {
        ...
        page.drawText(text, { x, y, size: fontSize, font: helveticaFont, ...options });
        return y - lineHeight;
    };
    ...
    addText("Solomon AI", margin + 30, height - margin - 25, { size: 16, font: helveticaBold });

    // Add title
    let y = addText(`Transaction Report for ${teamName}`, width / 2, height - margin - 60, {
        size: 24,
        font: helveticaBold,
    ...
    if (merchant) {
        y = addText(`Merchant: ${merchant}`, margin, y - lineHeight, { color: rgb(0.4, 0.4, 0.4) });
    }

    // Add generation date
    y = addText(`Generated on: ${format(new Date(), "MMMM d, yyyy 'at' HH:mm:ss")}`, margin, y - lineHeight, { color: rgb(0.4, 0.4, 0.4) });

    // Add summary
    const summary = calculateTransactionSummary(transactions);
    y -= lineHeight * 2;
    y = addText("Transaction Summary", margin, y, { font: helveticaBold, size: 16, color: rgb(0.2, 0.4, 0.6) });
    y = addText(`Total Amount: $${summary.totalAmount.toFixed(2)}`, margin, y - lineHeight);
    y = addText(`Number of Transactions: ${summary.transactionCount}`, margin, y - lineHeight);
    ...
    headers.forEach((header, index) => {
        let xPos = margin + columnWidths.slice(0, index).reduce((sum, width) => sum + width, 0);
    ...
    transactions.forEach((transaction, index) => {
        ...
        y = addText(transaction.category_slug || "Uncategorized", xPos, y + lineHeight);
...
function calculateTransactionSummary(transactions: Transaction[]): TransactionSummary {
    return transactions.reduce(
        (summary, transaction) => {
            summary.totalAmount += transaction.amount;
            summary.transactionCount += 1;
            summary.categories[transaction.category_slug || "Uncategorized"] =
                (summary.categories[transaction.category_slug || "Uncategorized"] || 0) + 1;
            summary.merchants[transaction.merchant_name || "N/A"] =
                (summary.merchants[transaction.merchant_name || "N/A"] || 0) + 1;
            return summary;
        },
        { totalAmount: 0, transactionCount: 0, categories: {}, merchants: {} } as TransactionSummary
    );
}
...
client.defineJob({
    ...
    run: async (payload, io, ctx) => {
        ...
        const { teamId, startDate, endDate, merchantName, transactionIds } = payload;

        await logger.info("Starting transaction export job", { teamId, startDate, endDate, merchantName });
        ...
        try {
            ...
            await logger.info("Retrieved transactions", { count: transactions.length });
            ...
            const { data: uploadData, error: uploadError } = await io.supabase.client.storage
                .from("vault")
            ...
            await logger.info("Job completed successfully", { downloadUrl: urlData.signedUrl });
            ...
            if (error instanceof Error) {
                await logger.error("Error in transaction export job", { error: error.message });
            } else {
                await logger.error("Error in transaction export job", { error: "An unknown error occurred" });
            }
            throw error;
        }
    },
