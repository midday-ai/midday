...
import { syncRecurringTransactionsSubTask } from "../subtasks/sync-recurring-transactions";
import { syncTransactionsSubTask } from "../subtasks/sync-transactions";
import { uniqueLog } from "../utils/log";
import { scheduler } from "./scheduler";

client.defineJob({
  id: Jobs.TRANSACTIONS_RECURRING_SYNC,
  name: "Recurring Transactions - Sync",
  version: "0.0.1",
  trigger: scheduler,
  ...
  run: async (_, io, ctx) => {
    ...
    try {
      ...
      await syncRecurringTransactionsSubTask(io, accountsData, prefix);
    } catch (error) {
