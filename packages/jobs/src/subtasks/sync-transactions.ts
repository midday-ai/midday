...
import type {
  TransactionsSchema as EngineTransaction,
  TransactionListParams,
} from "@solomon-ai/financial-engine-sdk/resources/transactions";
...
async function syncTransactionsSubTask(
  ...
  async function processAccount(account: BankAccountWithConnection) {
    try {
      ...
      const { transactions, cursor, hasMore } =
        await fetchTransactions(account);
      const formattedTransactions = await transformTransactions(
        transactions,
        account,
      );
      ...
      if (cursor) {
        ...
        if (error) {
          await uniqueLog(
            io,
            "error",
            `Failed to update bank connection ${account.bank_connection.id}: ${error.message}`,
          );
        } else {
          await uniqueLog(
            io,
            "info",
            `Successfully updated bank connection ${account.bank_connection.id} with new cursor`,
          );
        }
      } else {
        await uniqueLog(
          io,
          "warn",
          `No cursor returned for account ${account.id}, skipping bank connection update`,
        );
      }
      ...
      await uniqueLog(
        io,
        "info",
        `Completed processing for account ${account.id}. Has more: ${hasMore}`,
      );
    } catch (error) {
      await uniqueLog(
        io,
        "error",
        `Error processing account ${account.id}: ${error}`,
      );
    }
  }
  ...
  async function fetchTransactions(account: BankAccountWithConnection) {
    await uniqueLog(
      io,
      "info",
      `Fetching transactions for account ${account.id}`,
    );
    const accountType = getClassification(account.type);
    ...
    await uniqueLog(
      io,
      "info",
      `Retrieved ${transactions?.length || 0} transactions for account ${account.id} (Type: ${accountType}). Fetch request object ${JSON.stringify(request)}`,
    );
  ...
  async function transformTransactions(
    transactions: EngineTransaction.Data[],
    account: BankAccountWithConnection,
  ) {
    await uniqueLog(
      io,
      "info",
      `Transforming transactions for account ${account.id}`,
    );
    ...
    await processBatch(formattedTransactions, BATCH_LIMIT, async (batch) => {
      ...
      if (error) {
        ...
        await uniqueLog(
          io,
          "info",
          `Successfully upserted ${batch.length} transactions for account ${account.id}`,
        );
