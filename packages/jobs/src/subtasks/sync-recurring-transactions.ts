...
import type {
  ...
  TransactionRecurringParams,
  TransactionRecurringResponse,
...
async function syncRecurringTransactionsSubTask(
  ...
  async function processAccount(account: BankAccountWithConnection) {
    ...
    const { inflow: inflowTransactions, outflow: outflowTransactions } =
      transactions.data;
  ...
  ): Promise<TransactionRecurringResponse> {
    await uniqueLog(
      io,
      "info",
  ...
  async function upsertTransactions(
    ...
    let tinflow,
      toutflow: RecurringTransactionsForInsert[] = [];
    if (inflow) {
    ...
    const { error } = await supabase.rpc("insert_recurring_transactions", {
      p_data: { inflow: tinflow, outflow: toutflow },
    });
    if (error) {
      throw new Error(
        `Failed to upsert recurring transactions: ${error.message}`,
      );
    }
    ...
    await uniqueLog(
      io,
      "info",
      `Recurring transactions upserted successfully for account ${account.id}`,
    );
  }

  function transformTransactions(
    transactions: (
      | TransactionRecurringResponse.Inflow
      | TransactionRecurringResponse.Outflow
    )[],
    accountId: string,
  ): Array<RecurringTransactionsForInsert> {
