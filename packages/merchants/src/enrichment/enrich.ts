...
/**
 * Build context section from available merchant data
 */
function buildContextSection(params: EnrichMerchantParams): string {
  const parts: string[] = [];
...
/**
 * Enriches merchant data using a 2-step agent pipeline:
 *
...
export async function enrichMerchant(
  params: EnrichMerchantParams,
  options: EnrichMerchantOptions = {},
): Promise<EnrichMerchantResult> {
...
Step 2: Call extractData with companyName="${params.companyName}", domain="${domain}"`,
    ...
    let extractedData: MerchantEnrichmentResult | null = null;
    for (const tr of result.toolResults || []) {
      ...
      if (
        ...
        extractedData = output.extractedData as MerchantEnrichmentResult;
      }
    }
