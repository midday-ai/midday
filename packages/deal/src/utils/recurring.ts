...
/**
 * All valid recurring deal frequencies.
...
/**
 * All valid recurring deal statuses.
 * Used to derive both TypeScript types and Zod schemas.
 */
...
/**
 * All valid recurring deal end types.
 * Used to derive both TypeScript types and Zod schemas.
 */
export const RECURRING_END_TYPES = ["never", "on_date", "after_count"] as const;

/** Deal recurring frequency type derived from RECURRING_FREQUENCIES constant */
export type DealRecurringFrequency = (typeof RECURRING_FREQUENCIES)[number];

/** Deal recurring status type derived from RECURRING_STATUSES constant */
export type DealRecurringStatus = (typeof RECURRING_STATUSES)[number];

/** Deal recurring end type derived from RECURRING_END_TYPES constant */
export type DealRecurringEndType = (typeof RECURRING_END_TYPES)[number];
...
/**
 * Format recurring deal progress (e.g., "1 of 11")
 */
...
// They provide approximate next deal dates for display in the dashboard.
//
// IMPORTANT: These are simplified calculations without timezone support.
// The server-side calculations in @midday/db/utils/deal-recurring use
// proper timezone handling via @date-fns/tz and are the authoritative source
// for actual deal scheduling.
//
// Use these for:
// - Showing preview dates in the recurring deal configuration UI
// - Displaying upcoming deal calendars
// - Client-side summary calculations
//
// Do NOT use these for:
// - Actual deal scheduling (use server-side calculateNextScheduledDate)
...
export interface RecurringConfig {
  ...
  endType: DealRecurringEndType | null;
  endDate: string | null;
 ...
 * - Preview calendars in the recurring deal configuration
 *
 * For actual deal scheduling, the server uses `calculateNextScheduledDate`
 * from `@midday/db/utils/deal-recurring`, which includes proper timezone
...
/**
 * Calculate upcoming deal dates for preview (client-side version)
 */
...
/**
 * Calculate total deals and amount for the series
 */
...
export function validateRecurringConfig(
  ...
  if (config.endType === "after_count") {
    if (config.endCount === null || config.endCount === undefined) {
      errors.push({
        field: "endCount",
        message: "Deal count is required when ending after a count",
      });
    } else if (config.endCount < 1) {
      errors.push({
        field: "endCount",
        message: "Deal count must be at least 1",
      });
    }
  }
