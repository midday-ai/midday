...
export const manualSyncTransactionsAction = authActionClient
  ...
  .action(async ({ parsedInput: { connectionId, userId, accessToken, institutionId, institutionName, itemId }, ctx: { user } }) => {
    ...
    if (!backendExchangeResponse || !backendExchangeResponse.success) {
      logger("Failed to exchange access token with backend for Plaid", { itemId: itemId, accessToken: accessToken });
