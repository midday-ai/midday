...
export const exchangePublicToken = async (
  props: ExchangePublicTokenPropSchemaProps,
) => {
  ...
  try {
    ...
    const { publicToken, institutionId, institutionName, userId } =
      validatedProps;

    logger("Starting public token exchange", {
      institutionId,
      institutionName,
      userId,
    });
    ...
    logger("Plaid token exchange successful", {
      itemId: item_id,
      accessToken: access_token,
    });
