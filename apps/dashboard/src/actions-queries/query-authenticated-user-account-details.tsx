  ...
  try {
    ...
    if (!account?.supabaseAuth0UserId) {
      throw new APIRequestError(
        "Invalid account details or missing Supabase Auth User ID",
      );
    }
    ...
    const financialProfileRequest: GetUserProfile1Request = {
      ...
      profileType:
        request.profileType === GetUserV2ProfileTypeEnum.User
    ...
    const { profile: financialProfile, financialContext } =
      await backendClient.financialServiceApi.getUserProfile1(
    ...
    throw error instanceof APIRequestError
      ? error
      : new APIRequestError(
          "An unexpected error occurred while fetching user account details",
        );
  }
}
