...
async function LoadBankAccountData({
  ...
  const [bankConnection, accountBalance, user] = await Promise.all([
    getBankConnectionById(connectionId || ""),
    c.financialServiceApi.getAccountBalanceHistory({
  ...
  try {
    return (
      <Card className="w-full border-none bg-background text-foreground">
        <CardHeader>
          <CardTitle className="mt-2 text-3xl font-bold tracking-tight text-foreground sm:text-5xl">
