  ...
  persist(
    (set) => ({
      ...
      setUserAccount: (userAccount: UserAccount | BusinessAccount) =>
        set({ userAccount }),
      setUserFinancialProfile: (userFinancialProfile: FinancialUserProfile) =>
        set({ userFinancialProfile }),
      setUserFinancialContext: (userFinancialContext: MelodyFinancialContext) =>
        set({ userFinancialContext }),
      setToken: (token: string) => set({ token }),
      setData: (data: Partial<UserStateProps>) => set(data),
      setSupabaseAuth0UserId: (supabaseAuth0UserId: string) =>
        set({ supabaseAuth0UserId }),
