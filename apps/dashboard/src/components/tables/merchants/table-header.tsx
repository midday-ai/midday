...
export function DataTableHeader<TData>({
  ...
  const sortableColumnIds = useMemo(() => {
    ...
    return table
      .getAllLeafColumns()
      .filter((col) => !NON_REORDERABLE_COLUMNS.merchants.has(col.id))
      .map((col) => col.id);
  }, [table]);
        ...
        >
          ...
          >
            {headerGroup.headers.map((header, headerIndex, headers) => {
              ...
              const canReorder =
                !NON_REORDERABLE_COLUMNS.merchants.has(columnId);
...
function renderHeaderContent<TData>(
  ...
  const sortField = SORT_FIELD_MAPS.merchants[columnId];
