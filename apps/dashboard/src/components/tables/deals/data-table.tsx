...
export function DataTable({ initialSettings }: Props) {
  ...
  const { filter, hasFilters } = useDealFilterParams();
  const { setParams } = useDealParams();
  const { data: user } = useUserQuery();
  const parentRef = useRef<HTMLDivElement>(null);

  const { setColumns, setRowSelection, rowSelection } = useDealStore();

  // Hide header and summary grid on scroll
  useScrollHeader(parentRef, { extraOffset: SUMMARY_GRID_HEIGHTS.deals });
  ...
  const tableScroll = useTableScroll({
    ...
    startFromColumn: 2, // Skip sticky columns: select, dealNumber
  });
  ...
  const handleCellClick = useCallback(
    (rowId: string) => {
      setParams({ dealId: rowId, type: "details" });
    },
    [setParams],
  );
