...
export function DataTableHeader<TData>({
  ...
  const sortableColumnIds = useMemo(() => {
    ...
    return table
      .getAllLeafColumns()
      .filter((col) => !NON_REORDERABLE_COLUMNS.deals.has(col.id))
      .map((col) => col.id);
  }, [table]);
        ...
        >
          ...
          >
            {headerGroup.headers.map((header, headerIndex, headers) => {
              ...
              const canReorder =
                !NON_REORDERABLE_COLUMNS.deals.has(columnId);
...
function renderHeaderContent<TData>(
  ...
  const sortField = SORT_FIELD_MAPS.deals[columnId];
  const meta = header.column.columnDef.meta as
    | { headerLabel?: string }
    | undefined;
  ...
  // Deal Number column - special case with horizontal pagination
  if (columnId === "dealNumber") {
    return (
