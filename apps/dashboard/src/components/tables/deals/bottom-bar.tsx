...
import { useDownloadDealsZip } from "@/hooks/use-download-deals-zip";
...
export function BottomBar({ data }: Props) {
  const { rowSelection, setRowSelection } = useDealStore();
  const { handleDownloadZip, isPending, progress } = useDownloadDealsZip();

  // Filter the data array based on the selected row IDs (keys in rowSelection)
  const selectedDeals = data.filter((deal) => rowSelection[deal.id]);
  const downloadableDeals = selectedDeals; // Allow downloading all selected deals including drafts

  const handleBulkDownload = async () => {
    if (downloadableDeals.length === 0) {
      return;
    }

    await handleDownloadZip(downloadableDeals);
  };
          ...
          <div className="relative h-12 justify-between items-center flex pl-4 pr-2">
            ...
            <div className="flex items-center space-x-2">
              ...
              <SubmitButton
                ...
                disabled={downloadableDeals.length === 0}
              >
                <div className="flex items-center space-x-2">
                  <span>Download</span>
