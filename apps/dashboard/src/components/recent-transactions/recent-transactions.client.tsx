...
export function RecentTransactionsClient({
  ...
  const [transactions, setTransactions] =
    useState<TransactionSchema[]>(initialTransactions);
  ...
  if (!transactions.length) {
    return (
      <NoResults
        hasFilters={
          recurringTransactionFrequency !== undefined ||
  ...
  if (transactions && transactions.length > 0) {
    ...
    const firstTransactionDate = formatDate(
    ...
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" className="md:max-w-[300px] justify-start">
          ...
          <Icons.ChevronDown
            className={cn("ml-auto h-4 w-4 transition-transform duration-200", {
