...
import { fromStripeAmount } from "@midday/deal/currency";
...
export function PaymentModal({
  ...
  const initializePayment = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${apiUrl}/deal-payments/payment-intent`,
        {
          method: "POST",
          headers: {
  ...
  useOnClickOutside(panelRef as RefObject<HTMLElement>, (event) => {
    if (isDesktop && open && !useOverlay) {
      ...
      // Don't close if clicking inside the deal toolbar
      const toolbar = document.querySelector("[data-deal-toolbar]");
  ...
  const SuccessScreen = ({ isMobile = false }: { isMobile?: boolean }) => {
    ...
    const handleDownloadReceipt = async () => {
      ...
      try {
        await downloadFile(
          `${process.env.NEXT_PUBLIC_API_URL}/files/download/deal?token=${dealToken}&type=receipt`,
          `receipt-${dealNumber}.pdf`,
        );
