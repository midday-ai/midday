...
export function RecurringConfigPanel({
  ...
  // Calculate preview
  const previewDeals = React.useMemo(
  ...
  // Check if we need to show more deals indicator
  const hasMoreDeals =
  ...
  const handleEndTypeChange = (value: DealRecurringEndType) => {
    onChange({
      ...config,
      endType: value,
      endDate:
    ...
    <div className="space-y-4 min-w-[300px]">
      ...
      <div className="space-y-2">
        ...
        <RadioGroup
          ...
          onValueChange={(value) =>
            handleEndTypeChange(value as DealRecurringEndType)
          }
          className="space-y-3"
        >
          <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <Input
              ...
              className="w-16"
            />
            <span className="text-sm text-primary">deals</span>
          </div>
          <div className="flex items-center gap-2">
          ...
          >
              ...
              {previewDeals.map((deal, index) => (
                ...
                >
                  <div className="flex gap-3">
                    <span>{formatShortDate(deal.date)}</span>
                    <span className="text-muted-foreground">
                      {formatDayOfWeek(deal.date)}
                    </span>
                  </div>
                  <FormatAmount amount={deal.amount} currency={currency} />
                </div>
              ))}
              {hasMoreDeals && (
            ...
            <div className="border-t border-border pt-3 flex items-center justify-between text-sm">
              {summary.totalCount !== null && summary.totalAmount !== null ? (
                <>
                  <span>{summary.totalCount} deals total</span>
                  <FormatAmount
