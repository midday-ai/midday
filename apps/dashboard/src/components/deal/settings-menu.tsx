...
export function SettingsMenu() {
  ...
  const stripeOAuth = useAppOAuth({
    installUrlEndpoint: "/deal-payments/connect-stripe",
    onSuccess: () => {
      ...
      toast({
        ...
        description: "You can now accept payments for deals.",
      });
    },
    onError: () => {
      toast({
  ...
  const { data: templateCount } = useQuery(
    trpc.dealTemplate.count.queryOptions(),
  );
  ...
  const setDefaultMutation = useMutation(
    trpc.dealTemplate.setDefault.mutationOptions({
      onSuccess: () => {
  ...
  const deleteTemplateMutation = useMutation(
    trpc.dealTemplate.delete.mutationOptions({
      onSuccess: (data) => {
        queryClient.invalidateQueries({
          queryKey: trpc.dealTemplate.list.queryKey(),
        });
        queryClient.invalidateQueries({
          queryKey: trpc.dealTemplate.count.queryKey(),
        });

        // Switch to the new default template
        if (data?.newDefault) {
          setValue("template", data.newDefault, { shouldDirty: true });
          // Sync deal-level fields from the new template
  ...
  const duplicateTemplateMutation = useMutation(
    trpc.dealTemplate.create.mutationOptions({
      onSuccess: (data) => {
        queryClient.invalidateQueries({
          queryKey: trpc.dealTemplate.list.queryKey(),
        });
        ...
        if (data) {
          ...
          // Sync deal-level fields from the duplicated template
  ...
  const handleDuplicate = () => {
    ...
    // Get the CURRENT deal-level details (editors modify these, not template.*)
    ...
    duplicateTemplateMutation.mutate({
      ...templateSettings,
      // Override with current deal-level values to capture user's edits
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              cannot be undone. Existing deals using this template will not
              be affected.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              payments will be disabled for all deals.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
