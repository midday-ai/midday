...
export function SubmitButton({ isSubmitting, disabled }: Props) {
  ...
  const dealRecurringId = watch("dealRecurringId");

  // Check if deal is already part of a recurring series
  const isPartOfRecurringSeries = !!dealRecurringId;
  ...
  // Sync with form scheduledAt changes (for when deal data is loaded)
  React.useEffect(() => {
  ...
  const dealNumberValid = !formState.errors.dealNumber;
  ...
  const handleOptionChange = (value: string) => {
    ...
    if (deliveryType !== "scheduled" && deliveryType !== "recurring") {
      ...
      if (currentDeliveryType === "scheduled" && dealId) {
        cancelScheduleMutation.mutate({ id: dealId });
      }
    }
  ...
  const isValid = formState.isValid && dealNumberValid;

  // Build options based on deal state
  // - Hide "Recurring" if deal is already part of a recurring series
  // - Hide "Recurring" if deal is scheduled (can't convert scheduled to recurring)
  // - Hide "Schedule" if deal is already part of a recurring series
  const isScheduledDeal = selectedOption === "scheduled";
  const baseOptions = [
    {
  ...
  // Only show schedule option for non-recurring deals (recurring has its own scheduling)
  ...
  // 1. Deal is already part of a recurring series, OR
  // 2. Deal is currently scheduled (can't convert scheduled to recurring)
  const options =
    isPartOfRecurringSeries || isScheduledDeal
      ? optionsWithSchedule
      : [
          ...optionsWithSchedule,
          {
    ...
    <div className="flex flex-col gap-4">
      <div className="flex divide-x">
        ...
        {selectedOption === "scheduled" && canUpdate ? (
          // Show calendar and time input directly when deal is already scheduled and can be updated
          <DropdownMenu>
