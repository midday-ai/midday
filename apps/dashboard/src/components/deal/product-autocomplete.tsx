...
function extractTextFromValue(value: string): string {
  ...
  try {
    ...
    const extracted = extractTextFromNode(parsed).trim();
...
type Props = {
  ...
  onProductSelect?: (product: DealProduct) => void;
  disabled?: boolean;
};
...
export function ProductAutocomplete({
  ...
  const saveLineItemAsProductMutation = useMutation(
    trpc.dealProducts.saveLineItemAsProduct.mutationOptions({
      onSuccess: (result) => {
  ...
  const incrementUsageMutation = useMutation(
    trpc.dealProducts.incrementUsage.mutationOptions({
      onSuccess: () => {
    ...
    <div>
      <textarea
        ...
        className={cn(
          ...
          "text-primary leading-[18px] deal-editor overflow-hidden",
