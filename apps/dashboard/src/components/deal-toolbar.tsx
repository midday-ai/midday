...
import { PaymentModal } from "./deal/payment-modal";
...
export default function DealToolbar({
  ...
  const handleDownload = async () => {
    ...
    try {
      if (isPaid) {
        // For paid deals, download both deal and receipt as a zip
        const zip = new JSZip();
        const apiUrl = process.env.NEXT_PUBLIC_API_URL;

        // Fetch both PDFs in parallel for better performance
        const [dealResponse, receiptResponse] = await Promise.all([
          fetch(`${apiUrl}/files/download/deal?token=${token}`),
          fetch(`${apiUrl}/files/download/deal?token=${token}&type=receipt`),
        ]);
        ...
        const [dealBlob, receiptBlob] = await Promise.all([
          dealResponse.blob(),
          receiptResponse.blob(),
        ]);

        zip.file(`${dealNumber}.pdf`, dealBlob);
        zip.file(`receipt-${dealNumber}.pdf`, receiptBlob);
        ...
        saveAs(zipBlob, `${dealNumber}-deal-and-receipt.zip`);
      } else {
        // Download deal only
        await downloadFile(
          `${process.env.NEXT_PUBLIC_API_URL}/files/download/deal?token=${token}`,
          `${dealNumber}.pdf`,
        );
      }
    } catch (error) {
  ...
  // Show pay button only when deal can be paid (not paid, canceled, or refunded)
  const canPay =
    paymentEnabled &&
        ...
        >
          <TooltipProvider delayDuration={0}>
            <Tooltip>
              ...
              >
                <p>{isPaid ? "Download deal & receipt" : "Download"}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
