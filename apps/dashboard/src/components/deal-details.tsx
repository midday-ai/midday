...
import { DealDetailsSkeleton } from "./deal-details-skeleton";
...
export function DealDetails() {
  const trpc = useTRPC();
  const { dealId } = useDealParams();
  const { data: user } = useUserQuery();

  const isOpen = dealId !== null;

  const { data, isLoading } = useQuery({
    ...trpc.deal.getById.queryOptions({ id: dealId! }),
    enabled: isOpen,
  });

  // Fetch upcoming deals for recurring series
  const { data: upcomingDeals } = useQuery({
    ...trpc.dealRecurring.getUpcoming.queryOptions({
      id: data?.dealRecurringId ?? "",
      limit: 5,
    }),
    enabled: !!data?.dealRecurringId && data?.recurring?.status === "active",
  });
  ...
  return (
    <div className="h-full">
      ...
      <DealActions
        ...
        dealRecurringId={dealRecurringId}
      ...
      <div className="h-full p-0 pb-[143px] overflow-y-auto scrollbar-hide">
        ...
        <div className="mt-6 flex flex-col space-y-4 border-t border-border pt-6">
          ...
          <div className="flex justify-between items-center">
            <span className="text-sm text-[#606060]">Deal no.</span>
            <span className="text-sm">
        ...
        {dealRecurringId && recurring && (
          <div className="mt-6 border-t border-border pt-6">
            ...
            {upcomingDeals?.deals &&
              upcomingDeals.deals.length > 0 && (
                <div className="mt-4 pt-4 border-t border-border space-y-3">
                  {upcomingDeals.deals
                    .slice(0, recurring.endCount ? 5 : 3)
                    .map(
                      (
                        deal: { date: string; amount: number },
                        index: number,
                      ) => {
                        // Check if this is the first deal in the series and it's scheduled
                        const isCurrentScheduledDeal =
                          index === 0 &&
                          status === "scheduled" &&
                  ...
                  {/* Show ellipsis if there are more deals */}
                  {((recurring.endCount &&
                    recurring.endCount - recurring.dealsGenerated > 5) ||
                    !recurring.endCount) && (
            ...
            <div className="flex justify-between text-sm mt-4 pt-4 border-t border-border">
              {upcomingDeals &&
              upcomingDeals.summary?.totalCount !== null &&
              upcomingDeals.summary?.totalAmount !== null ? (
                <>
                  <span>
                    {upcomingDeals.summary.totalCount} deals total
                  </span>
                  <FormatAmount
                    amount={upcomingDeals.summary.totalAmount}
                    currency={currency ?? "USD"}
                  />
                </>
                ...
                <>
                  ...
                  <span className="text-muted-foreground">
                    {recurring.dealsGenerated} generated
                  </span>
                </>
              ) : recurring.endType === "after_count" && recurring.endCount ? (
                <>
                  <span className="text-muted-foreground">
                    {recurring.dealsGenerated} of {recurring.endCount}{" "}
                    deals
                  </span>
                  <span className="text-muted-foreground">
                    {recurring.endCount - recurring.dealsGenerated} remaining
                  </span>
                </>
              ) : (
                <>
        ...
        {merchant && (
          <div className="mt-6 flex flex-col space-y-2 border-t border-border pt-6">
            <span className="text-sm text-[#606060]">Deal link</span>
            <div className="flex w-full gap-2">
              ...
              {status !== "draft" && (
                <Button
                  ...
                  onClick={() => {
                    if (downloadUrl) {
                      downloadFile(downloadUrl, `${dealNumber}.pdf`);
                    }
                  }}
        ...
        >
          <AccordionItem value="activity">
            ...
            <AccordionContent>
              <DealActivity data={data} />
            </AccordionContent>
          </AccordionItem>
          <AccordionItem value="note">
            <AccordionTrigger>Internal note</AccordionTrigger>
            <AccordionContent>
              <DealNote id={id} defaultValue={internalNote} />
            </AccordionContent>
          </AccordionItem>
        </Accordion>
      </div>
