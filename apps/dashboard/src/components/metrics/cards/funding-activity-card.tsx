...
import { FundingActivityChart } from "@/components/charts/funding-activity-chart";
...
export function FundingActivityCard({
  from,
  to,
  currency,
  locale,
  ...
  const { data } = useQuery(
    trpc.reports.fundingActivity.queryOptions({
      from,
      to,
      currency: currency,
    }),
  ...
  const heroKpi = useMemo(() => {
    return data?.summary?.totalFunded ?? 0;
  }, [data]);
      ...
      <div className="mb-4 min-h-[140px]">
        ...
        <div className="flex items-center gap-4 mt-2">
          <div className="flex gap-2 items-center">
            ...
            <span className="text-xs text-muted-foreground">Amount Funded</span>
          </div>
          <div className="flex gap-2 items-center">
            <div
              className="w-4 h-0.5"
            ...
            />
            <span className="text-xs text-muted-foreground">Deal Count</span>
          </div>
        </div>
      </div>
      <div className="h-80">
        <FundingActivityChart
          data={chartData}
          height={320}
          currency={currency}
          locale={locale}
