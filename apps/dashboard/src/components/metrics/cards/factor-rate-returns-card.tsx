...
import { FactorRateReturnsChart } from "@/components/charts/factor-rate-returns-chart";
...
interface FactorRateReturnsCardProps {
  from: string;
  to: string;
  currency?: string;
  locale?: string;
...
export function FactorRateReturnsCard({
  from,
  to,
  currency,
  locale,
  ...
  const { data } = useQuery(
    trpc.reports.factorRateReturns.queryOptions({
      from,
      to,
      currency: currency,
    }),
      ...
      <div className="mb-4 min-h-[140px]">
        ...
        <div className="flex items-center gap-4 mt-2">
          ...
          <div className="flex gap-2 items-center">
            ...
            />
            <span className="text-xs text-muted-foreground">Avg Factor Rate</span>
          </div>
        </div>
      </div>
      <div className="h-80">
        <FactorRateReturnsChart
          data={chartData}
          height={320}
          currency={currency}
          locale={locale}
