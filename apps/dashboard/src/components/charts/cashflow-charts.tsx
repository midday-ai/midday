...
export function CashflowCharts({ currency, disabledCharts = [], disableAllCharts = false }: CashflowChartsProps) {
    const chartOpacity = (chartName: ChartType) => disabledCharts.includes(chartName) ? "opacity-50" : "";

    return (
        <div className={cn("grid grid-cols-1 gap-4 mx-auto py-[2%]", disableAllCharts ? "opacity-50" : "")}>
            <CardWrapper
                title="Net Income"
