...
export function PaymentChannelSpendingCategoryList({
  data,
  period,
  disabled,
  currency = "USD",
}: Props) {
  const [progressType, setProgressType] = useState<"expense" | "count">(
    "expense",
  );
  ...
  return (
    <>
      <div className="flex justify-end mb-4 py-2">
        <Select
          onValueChange={(value) =>
            setProgressType(value as "expense" | "count")
          }
      ...
      <ul className="mt-4 space-y-4 overflow-auto scrollbar-hide aspect-square pb-14">
        {data.map(({ payment_channel, total_expense, transaction_count }) => {
          const progressValue =
          ...
          return (
            <li key={payment_channel}>
              <HoverCard openDelay={10} closeDelay={10}>
                <HoverCardTrigger asChild>
                  <Link
                    className="flex items-center"
                    href={`/transactions?payment_channel=${payment_channel}&start=${period?.from}&end=${period?.to}`}
                  >
                    <Category
                      key={payment_channel}
