...
export async function ExpenseGrowthRateChart({
  ...
  const result = await getExpenseGrowthRate({
    ...defaultValue,
    ...value,
    currency,
  });
  ...
  const mostPositiveGrowthRate = result?.data?.reduce(
    (max: any, item: any) => (item.growth_rate > max.growth_rate ? item : max),
    result?.data[0],
  );
  const mostNegativeGrowthRate = result?.data?.reduce(
    (min: any, item: any) => (item.growth_rate < min.growth_rate ? item : min),
    result?.data[0],
  );
  // get the absolute value of the largest growth rate
  const largestGrowthRate = Math.max(
    Math.abs(mostPositiveGrowthRate.growth_rate),
    Math.abs(mostNegativeGrowthRate.growth_rate),
  );
  // figure out the sign of the largest growth rate
  const largestGrowthRateSign =
    mostPositiveGrowthRate.growth_rate >
    Math.abs(mostNegativeGrowthRate.growth_rate)
      ? "+"
      : "-";
  ...
  const averageExpense =
    result?.data?.reduce((sum: any, item: any) => sum + item.total_expense, 0) /
        ...
        <div className="text-sm text-[#606060] flex items-center space-x-2">
          <p className="text-sm">
            ...
            Math.abs(mostNegativeGrowthRate.growth_rate)
              ? "Expenses increased"
              : "Expenses decreased"}{" "}
            by {largestGrowthRate.toFixed(2)}% compared to the previous period.
          </p>
