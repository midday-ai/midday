...
export async function MonthlyExpenseChart({
    ...
    const result = await getMonthlyExpenses({ ...defaultValue, ...value, currency });

    if (!result?.data) {
        return null;
    }

    const dataPoints: { date: string, value: number }[] = result.data.map((item: ExpenseData) => ({
        date: item.month,
    ...
    return (
        <div className={cn(disabled && "pointer-events-none select-none", "overflow-y-auto scrollbar-hide")}>
            <CardHeader className="flex flex-col gap-2">
                <div className="flex justify-between">
                    <CardTitle className="font-bold text-2xl md:text-4xl">Monthly Expense Breakdown</CardTitle>
...
function computeExpenseStatistics(data: ExpenseData[], currency: string): ExpenseStatistics {
    ...
    const monthWithMaxExpense = data.find(item => item.total_expense === maxExpense)?.month ?? '';
    const monthWithMinExpense = data.find(item => item.total_expense === minExpense)?.month ?? '';

    const expenseGrowthRate = expenses.length > 1 && expenses[0] !== 0
        ? ((expenses[expenses.length - 1]! - expenses[0]!) / expenses[0]!) * 100
        : 0;

    const squaredDiffs = expenses.map(expense => Math.pow(expense - averageExpense, 2));
    const avgSquaredDiff = squaredDiffs.reduce((sum, diff) => sum + diff, 0) / expenses.length;
    ...
    return (
        <div className="space-y-4 mb-14 select-text">
            <div className="gap-2 flex flex-1 overflow-x-auto scrollbar-hide">
                <StatItem label="Average Monthly Expense" value={stats.averageExpense} currency={stats.currency} />
                <StatItem label="Total Expenses" value={stats.totalExpense} currency={stats.currency} />
                <StatItem label="Median Monthly Expense" value={stats.medianExpense} currency={stats.currency} />
                <StatItem label="Highest Monthly Expense" value={stats.maxExpense} currency={stats.currency} subLabel={stats.monthWithMaxExpense} />
                <StatItem label="Lowest Monthly Expense" value={stats.minExpense} currency={stats.currency} subLabel={stats.monthWithMinExpense} />
                <StatItem label="Expense Growth Rate" value={stats.expenseGrowthRate} suffix="%" />
                <StatItem label="Expense Volatility" value={stats.volatility} currency={stats.currency} />
            </div>
        </div>
    );
}

function StatItem({ label, value, currency, suffix, subLabel }: { label: string; value: number; currency?: string; suffix?: string; subLabel?: string }) {
    return (
...
function MonthlyBreakdown({ breakdown, currency }: { breakdown: Array<{ month: string; total: number; categories: { [category: string]: number } }>; currency: string }) {
    return (
        <div className="mt-8">
            ...
            <div className="space-y-4">
                {breakdown.map(({ month, total, categories }) => (
                    <div key={month} className="bg-gray-100 p-4 rounded-lg">
                        <h4 className="text-lg font-medium mb-2">{month} - Total: <AnimatedNumber value={total} currency={currency} /></h4>
                        <ul className="space-y-1">
                            {Object.entries(categories).map(([category, amount]) => (
                                <li key={category} className="flex justify-between text-sm">
                                    <span>{category}</span>
                                    <span><AnimatedNumber value={amount} currency={currency} /></span>
                                </li>
                            ))}
                        </ul>
                    </div>
