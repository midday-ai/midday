...
const useExpenseMetrics = (
  userId: string,
  pageNumber: string,
  pageSize: string,
  selectedCategory: string,
) => {
  const [expenseMetrics, setExpenseMetrics] = useState<ExpenseMetrics[] | null>(
    null,
  );
...
const ExpenseMetricsChartContent: React.FC<ExpenseMetricsChartProps> = ({
  ...
  const { expenseMetrics, isLoading, error } = useExpenseMetrics(
    userId,
    pageNumber,
    pageSize,
    selectedCategory,
  ...
  const transformExpenseData = useCallback(
    (expenseMetrics: ExpenseMetrics[]): DataPoint[] => {
  ...
  const expenseMetricsData: Array<ExpenseMetrics> = expenseMetrics.map(
    (metric) => ({
      ...metric,
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <ZoomableChartWithDrilldown
        ...
        footerDescription={
          hasData
            ? `Total expenses (${formatCategoryName(selectedCategory)})`
            : "No expense data available for the selected period"
        }
        chartType="area"
        description={
          hasData
            ? `Monthly expenses (${formatCategoryName(selectedCategory)})`
      ...
      />
      <NetExpenseChart
        title={"Net Expense Metrics"}
        price={
          expenseMetricsData[expenseMetricsData.length - 1]?.totalExpenses ?? 0
        }
        priceChange={0}
