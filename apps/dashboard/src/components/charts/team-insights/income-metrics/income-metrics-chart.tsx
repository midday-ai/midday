...
} from "@midday/ui/charts/base/zoomable-chart-with-drilldown";
...
const useIncomeMetrics = (
  userId: string,
  pageNumber: string,
  pageSize: string,
  selectedCategory: string,
) => {
  const [IncomeMetrics, setIncomeMetrics] = useState<IncomeMetrics[] | null>(
    null,
  );
...
const IncomeMetricsChartContent: React.FC<IncomeMetricsChartProps> = ({
  ...
  const { IncomeMetrics, isLoading, error } = useIncomeMetrics(
    userId,
    pageNumber,
    pageSize,
    selectedCategory,
  ...
  const categories = useMemo(() => {
    ...
    return [
      ...
      ...new Set(
        IncomeMetrics.map(
          (metric) => metric.personalFinanceCategoryPrimary,
        ).filter(Boolean),
      ),
    ];
  }, [IncomeMetrics]);
  ...
  const transformExpenseData = useCallback(
    (IncomeMetrics: IncomeMetrics[]): DataPoint[] => {
      return IncomeMetrics.map((expense) => ({
  ...
  const incomeMetricsData: Array<IncomeMetrics> = IncomeMetrics.map(
    (metric) => ({
      ...metric,
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex flex-col gap-4 text-4xl py-[5%] font-bold justify-start items-start w-[50%]">
        Your expenses for the period from {effectiveFrom} to {effectiveTo}{" "}
        across the{" "}
        {formatCategoryName(selectedCategory as string).toLocaleLowerCase()}{" "}
        category
      </div>
      ...
      <ZoomableChartWithDrilldown
        data={
          hasData
            ? data
            : [
                { date: effectiveFrom, events: 0 },
                { date: effectiveTo, events: 0 },
              ]
        }
        dataNameKey="income"
        height={600}
        footerDescription={
          hasData
            ? `Total income (${formatCategoryName(selectedCategory)})`
            : "No expense data available for the selected period"
        }
        chartType="area"
        description={
          hasData
            ? `Monthly income (${formatCategoryName(selectedCategory)})`
            : "No income recorded"
        }
      ...
      />
      <NetIncomeChart
        title={"Net Income Metrics"}
        price={
          incomeMetricsData[incomeMetricsData.length - 1]?.totalIncome ?? 0
        }
        priceChange={0}
