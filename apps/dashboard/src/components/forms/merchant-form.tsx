...
type Props = {
  data?: RouterOutputs["merchants"]["getById"];
};

export function MerchantForm({ data }: Props) {
  const trpc = useTRPC();
  ...
  const upsertMerchantMutation = useMutation(
    trpc.merchants.upsert.mutationOptions({
      onSuccess: (data) => {
        ...
        queryClient.invalidateQueries({
          queryKey: trpc.merchants.get.queryKey(),
        });

        queryClient.invalidateQueries({
          queryKey: trpc.merchants.getById.queryKey(),
        });
  ...
  const handleSubmit = (data: z.infer<typeof formSchema>) => {
    ...
    upsertMerchantMutation.mutate(formattedData);
  };
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)}>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="mt-6">
                      ...
                      <FormDescription className="mt-2">
                        Tags help categorize and track merchant activity.
                      </FormDescription>
                    </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4">
          <div className="flex justify-end mt-auto space-x-4">
            <Button
              variant="outline"
              onClick={() => setMerchantParams(null)}
              type="button"
            >
              Cancel
            </Button>
            ...
            <SubmitButton
              isSubmitting={upsertMerchantMutation.isPending}
              disabled={
                upsertMerchantMutation.isPending || !form.formState.isDirty
              }
            >
