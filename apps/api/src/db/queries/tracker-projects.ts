  ...
  return await db.transaction(async (tx) => {
    ...
    const [result] = await tx
      ...
      .values({
        ...
        rate:
          projectData.rate !== undefined ? String(projectData.rate) : undefined,
      ...
      .onConflictDoUpdate({
        ...
        set: {
          ...
          // Convert numeric types if needed
          rate:
            projectData.rate !== undefined
              ? String(projectData.rate)
              : undefined,
          // Convert estimate to bigint number mode
          estimate:
...
export async function getTrackerProjectById(
  ...
  const [assignedUsersResult] = await db.execute(
