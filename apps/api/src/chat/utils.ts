...
const WORD_COUNT_SIMPLE = 15;
const WORD_COUNT_COMPLEX = 40;

export function classifyComplexity(uiMessages: UIMessage[]): boolean {
  const lastUser = [...uiMessages].reverse().find((m) => m.role === "user");
  if (!lastUser) return true;

  const text = userMessageText(lastUser).toLowerCase();
  const words = text.split(/\s+/).filter(Boolean);

  if (words.length > WORD_COUNT_COMPLEX) return true;

  if (COMPLEX_PATTERNS.some((p) => text.includes(p))) return true;

  if (
    words.length <= WORD_COUNT_SIMPLE &&
    SIMPLE_PATTERNS.some((p) => text.startsWith(p))
  ) {
    return false;
  }
