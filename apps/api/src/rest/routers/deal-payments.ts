...
import { toStripeAmount } from "@midday/deal/currency";
import { verify as verifyDealToken } from "@midday/deal/token";
...
interface DealPaymentOAuthState {
  teamId: string;
  userId: string;
  source: "deal-settings";
}

function isValidOAuthState(
  parsed: unknown,
): parsed is DealPaymentOAuthState {
  return (
    typeof parsed === "object" &&
    parsed !== null &&
    typeof (parsed as Record<string, unknown>).teamId === "string" &&
    typeof (parsed as Record<string, unknown>).userId === "string" &&
    (parsed as Record<string, unknown>).source === "deal-settings"
  );
}
...
app.openapi(
  createRoute({
    ...
    description:
      "Generates OAuth URL for Stripe Connect Standard integration. Allows teams to connect their Stripe account for accepting deal payments.",
    tags: ["Deal Payments"],
    responses: {
      200: {
  ...
  async (c) => {
    ...
    const redirectUri = `${process.env.ABACUS_API_URL || "https://api.abacuslabs.co"}/deal-payments/connect-stripe/callback`;
...
app.openapi(
  ...
  async (c) => {
    ...
    if (error || !code) {
      ...
      return c.redirect(
        `${dashboardUrl}/deals?stripe_connect=error&message=${encodeURIComponent(error_description || error || "Connection cancelled")}`,
        302,
      );
    }
    ...
    try {
      ...
      return c.redirect(
        `${dashboardUrl}/deals?stripe_connect=error&message=${encodeURIComponent("Failed to connect Stripe account")}`,
        302,
      );
    }
  },
...
// ============================================================================
// Create Payment Intent Endpoint (Public - for deal customers)
// ============================================================================

const createPaymentIntentSchema = z.object({
  token: z.string().openapi({
    description: "Deal token for authentication",
  }),
});
...
app.openapi(
  createRoute({
    method: "post",
    path: "/payment-intent",
    summary: "Create payment intent for deal",
    operationId: "createDealPaymentIntent",
    description:
      "Creates a Stripe PaymentIntent for paying an deal. This is a public endpoint that uses the deal token for authentication.",
    tags: ["Deal Payments"],
    request: {
      body: {
        content: {
    ...
    responses: {
      ...
      400: {
        description: "Invalid request or deal not payable",
      },
      404: {
  ...
  async (c) => {
    ...
    // Verify token and get deal ID
    let dealId: string;
    try {
      const { id } = (await verifyDealToken(decodeURIComponent(token))) as {
        id: string;
      };
      if (!id) {
      ...
      throw new HTTPException(400, { message: "Invalid deal token" });
    }
    ...
    // Check if deal can be paid
    if (deal.status === "paid") {
      throw new HTTPException(400, { message: "Deal is already paid" });
    }

    if (deal.status === "draft") {
      throw new HTTPException(400, { message: "Deal is still a draft" });
    }

    if (deal.status === "canceled") {
      throw new HTTPException(400, { message: "Deal has been canceled" });
    }

    // Check if payment is enabled for this deal's template
    const template = deal.template as { paymentEnabled?: boolean } | null;
    if (!template?.paymentEnabled) {
      throw new HTTPException(400, {
        message: "Online payment is not enabled for this deal",
      });
    }

    // Get team to retrieve Stripe account
    const team = await getTeamById(db, deal.teamId);
    ...
    // Calculate amount in Stripe's smallest currency unit (cents)
    const amount = toStripeAmount(deal.amount || 0);

    if (amount <= 0) {
      throw new HTTPException(400, { message: "Invalid deal amount" });
    }

    try {
      const stripe = new Stripe(process.env.STRIPE_SECRET_KEY!);

      // Check if there's an existing payment intent for this deal
      if (deal.paymentIntentId) {
        try {
          ...
          // This catches edge case where webhook hasn't updated deal status yet
          if (existingIntent.status === "succeeded") {
            throw new HTTPException(400, {
              message: "Deal has already been paid",
            });
          }
          ...
          if (
            ...
            // Check if the deal amount has changed since the payment intent was created
      ...
      // arrive simultaneously for the same deal, Stripe will only create
      // one payment intent and return the same result to both requests.
      const idempotencyKey = `deal_payment_${deal.id}_${amount}_usd`;
...
export { app as dealPaymentsRouter };
