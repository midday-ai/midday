...
export function isAgentMode(flags: { json?: boolean; agent?: boolean; quiet?: boolean }): boolean {
  if (flags.agent || flags.json) return true;
  if (!isTTY()) return true;
  return false;
}

export function shouldShowUI(flags: { json?: boolean; agent?: boolean; quiet?: boolean }): boolean {
