...
export function createInvoicesCommand(): Command {
  const cmd = new Command("invoices").description("Create, send, and track invoices");
  ...
  cmd
    ...
    .option("--status <status>", "Filter by status (draft, unpaid, paid, overdue, canceled)")
      ...
      try {
        ...
        if (format === "json") {
          ...
          const rows = invoices.map((inv) => [
            ...
            inv.amount != null ? formatAmount(inv.amount, inv.currency || "USD") : null,
            inv.status || null,
            inv.due_date || null,
          ]);
    ...
    .action(async (id: string) => {
      ...
      try {
        const inv = await withSpinner(
          "Fetching invoice...",
          () => get<Invoice>(`/invoices/${id}`, undefined, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
        ...
        if (format === "json") {
          ...
          printDetail(`Invoice ${inv.invoice_number || id}`, [
            ...
            ["Amount", inv.amount != null ? formatAmount(inv.amount, inv.currency || "USD") : null],
            ["Status", inv.status],
      ...
      try {
        ...
        if (opts.stdin) {
          ...
          body = {
            ...
            issue_date: opts.issueDate || new Date().toISOString().split("T")[0],
        ...
        const inv = await withSpinner(
          "Creating invoice...",
          () => post<Invoice>("/invoices", body, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
        ...
        if (format === "json") {
          ...
          if (inv.customer_name) console.log(`  ${chalk.dim("Customer:")} ${inv.customer_name}`);
          console.log();
        }
      } catch (error) {
        handleError(error, format);
  ...
  midday invoices send inv_abc123 --yes`,
    ...
    .action(async (id: string, opts) => {
      ...
      try {
        ...
        const inv = await withSpinner(
          "Sending invoice...",
          () => post<Invoice>(`/invoices/${id}/send`, undefined, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
  ...
  cat update.json | midday invoices update inv_abc123 --stdin`,
    ...
    .action(async (id: string, opts) => {
      ...
      try {
        ...
        const inv = await withSpinner(
          "Updating invoice...",
          () => put<Invoice>(`/invoices/${id}`, body, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
  ...
  midday invoices delete inv_abc123 --dry-run`,
    ...
    .action(async (id: string, opts) => {
      ...
      try {
        ...
        await withSpinner(
          "Deleting invoice...",
          () => del(`/invoices/${id}`, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
  ...
  midday invoices mark-paid inv_abc123 --dry-run`,
    ...
    .action(async (id: string, opts) => {
      ...
      try {
        ...
        const inv = await withSpinner(
          "Marking as paid...",
          () => post<Invoice>(`/invoices/${id}/mark-paid`, undefined, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
  ...
  midday invoices remind inv_abc123 --dry-run`,
    ...
    .action(async (id: string, opts) => {
      ...
      try {
        ...
        await withSpinner(
          "Sending reminder...",
          () => post(`/invoices/${id}/remind`, undefined, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
