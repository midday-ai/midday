...
import { get, put, del, post } from "../../client/api.js";
...
export function createInboxCommand(): Command {
  const cmd = new Command("inbox").description("Process receipts and match to transactions");
    ...
    .action(async (opts) => {
      ...
      try {
        const data = await withSpinner(
          "Fetching inbox...",
          () =>
            get<{ data: InboxItem[]; meta?: { cursor?: string; hasMore?: boolean; count?: number } }>("/inbox", {
              cursor: opts.cursor,
        ...
        if (format === "json") {
          ...
          const rows = items.map((item) => [
            ...
            item.amount != null ? formatAmount(item.amount, item.currency || "USD") : null,
            item.date || null,
            item.status || null,
            item.transaction_id ? chalk.green("matched") : chalk.dim("unmatched"),
          ]);
    ...
    .action(async (id: string) => {
      ...
      try {
        const item = await withSpinner(
          "Fetching inbox item...",
          () => get<InboxItem>(`/inbox/${id}`, undefined, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
        ...
        if (format === "json") {
          ...
          printDetail(item.display_name || id, [
            ["ID", item.id],
            ["Amount", item.amount != null ? formatAmount(item.amount, item.currency || "USD") : null],
            ["Date", item.date],
            ["Status", item.status],
    ...
    .action(async (inboxId: string, opts) => {
      ...
      try {
        await withSpinner(
          "Matching...",
          () =>
            post(`/inbox/${inboxId}/match`, { transaction_id: opts.transaction }, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );

        if (format === "json") {
          printJson({ matched: true, inbox_id: inboxId, transaction_id: opts.transaction });
        } else {
  ...
  midday inbox delete inb_abc123 --yes`,
    ...
    .action(async (id: string) => {
      ...
      try {
        await withSpinner(
          "Deleting inbox item...",
          () => del(`/inbox/${id}`, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
