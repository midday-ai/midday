...
import { loginWithBrowser, loginWithToken, logout, getAuthStatus } from "../../client/auth.js";
...
export function createAuthCommand(): Command {
  const auth = new Command("auth").description("Login, logout, check auth status");
    ...
    .action(async (opts: { tokenStdin?: boolean; browser?: boolean }) => {
      ...
      try {
        if (opts.tokenStdin) {
          ...
          await withSpinner("Verifying token...", () => loginWithToken(token), globals.quiet);
        } else {
        ...
        if (format === "json") {
          printJson({ authenticated: true, email: status.email, team: status.teamName });
        } else {
          ...
          console.log(`  ${chalk.cyan("midday transactions list")}    View recent transactions`);
          console.log(`  ${chalk.cyan("midday invoices list")}        View your invoices`);
          console.log(`  ${chalk.cyan("midday tracker start")}        Start tracking time`);
          console.log(`  ${chalk.cyan("midday inbox list")}            View your inbox`);
          console.log(`  ${chalk.cyan("midday --help")}               See all commands`);
          console.log();
        }
      } catch (error) {
