    ...
    .action(async () => {
      ...
      try {
        const data = await withSpinner(
          "Fetching products...",
          () => get<{ data: Product[] }>("/invoice-products", undefined, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
        ...
        if (format === "json") {
          ...
          printTable({ title: `Products (${products.length})`, head: ["Name", "Price", "Unit"], rows });
        }
      } catch (error) {
    ...
    .action(async (opts) => {
      ...
      try {
        ...
        const product = await withSpinner(
          "Creating product...",
          () => post<Product>("/invoice-products", body, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
  ...
  midday products update prod_abc123 --name "Senior Consulting" --price 200`,
    ...
    .action(async (id: string, opts) => {
      ...
      try {
        ...
        const product = await withSpinner(
          "Updating product...",
          () => put<Product>(`/invoice-products/${id}`, body, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
  ...
  midday products delete prod_abc123`,
    ...
    .action(async (id: string) => {
      ...
      try {
        await withSpinner(
          "Deleting product...",
          () => del(`/invoice-products/${id}`, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
