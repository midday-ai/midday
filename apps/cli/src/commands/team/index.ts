    ...
    .action(async () => {
      ...
      try {
        const team = await withSpinner(
          "Fetching team info...",
          () => get<Team>("/teams/current", undefined, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
    ...
    .action(async () => {
      ...
      try {
        const data = await withSpinner(
          "Fetching members...",
          () => get<{ data: Member[] }>("/teams/current/members", undefined, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
        ...
        if (format === "json") {
          ...
          const rows = members.map((m) => [m.full_name || null, m.email, m.role || null]);
