    ...
    .action(async () => {
      ...
      try {
        const data = await withSpinner(
          "Fetching tags...",
          () => get<{ data: Tag[] }>("/tags", undefined, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
        ...
        if (format === "json") {
          ...
          printTable({ title: `Tags (${tags.length})`, head: ["Name", "ID"], rows });
        }
      } catch (error) {
    ...
    .action(async (opts) => {
      ...
      try {
        const tag = await withSpinner(
          "Creating tag...",
          () => post<Tag>("/tags", { name: opts.name }, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
  ...
  midday tags delete tag_abc123`,
    ...
    .action(async (id: string) => {
      ...
      try {
        await withSpinner(
          "Deleting tag...",
          () => del(`/tags/${id}`, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
