...
export function createTransactionsCommand(): Command {
  const cmd = new Command("transactions").description("List, search, and manage transactions");
    ...
    .action(async (id: string) => {
      ...
      try {
        const txn = await withSpinner(
          ...
          () => get<Transaction>(`/transactions/${id}`, undefined, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
    ...
    .action(async (opts) => {
      ...
      try {
        ...
        const txn = await withSpinner(
          ...
          () => post<Transaction>("/transactions", body, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
        ...
        if (format === "json") {
          ...
          console.log(`  ${chalk.dim("Amount:")} ${formatAmount(txn.amount, txn.currency)}\n`);
        }
      } catch (error) {
        handleError(error, format);
      }
    ...
    .action(async (id: string, opts) => {
      ...
      try {
        ...
        const txn = await withSpinner(
          ...
          () => put<Transaction>(`/transactions/${id}`, body, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
    ...
    .action(async (id: string, opts) => {
      ...
      try {
        ...
        await withSpinner(
          ...
          () => del(`/transactions/${id}`, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
