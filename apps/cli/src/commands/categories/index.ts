...
export function createCategoriesCommand(): Command {
  const cmd = new Command("categories").description("Manage transaction categories");
    ...
    .action(async () => {
      ...
      try {
        const data = await withSpinner(
          "Fetching categories...",
          () => get<{ data: Category[] }>("/categories", undefined, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
        ...
        if (format === "json") {
          ...
          const rows = categories.map((c) => [c.name, c.slug, c.description || null]);
    ...
    .action(async (opts) => {
      ...
      try {
        ...
        const cat = await withSpinner(
          "Creating category...",
          () => post<Category>("/categories", body, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
  ...
  midday categories update cat_abc123 --color "#10B981"`,
    ...
    .action(async (id: string, opts) => {
      ...
      try {
        ...
        const cat = await withSpinner(
          "Updating category...",
          () => put<Category>(`/categories/${id}`, body, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
  ...
  midday categories delete cat_abc123 --yes`,
    ...
    .action(async (id: string) => {
      ...
      try {
        await withSpinner(
          "Deleting category...",
          () => del(`/categories/${id}`, { apiUrl: globals.apiUrl, debug: globals.debug }),
          globals.quiet,
        );
