...
import { saveCredentials, clearCredentials, getCredentials } from "../config/store.js";
...
async function registerClient(apiUrl: string, redirectUri: string): Promise<{ clientId: string }> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Client registration failed: ${response.statusText} — ${body}`);
  }
...
function startCallbackServer(): Promise<{
  ...
  return new Promise((resolveServer, rejectServer) => {
    ...
    const codePromise = new Promise<{ code: string; close: () => void }>((res, rej) => {
      resolveCode = res;
      rejectCode = rej;
    });
...
export async function loginWithBrowser(opts: { noBrowser?: boolean } = {}): Promise<void> {
  ...
  const tokens = await exchangeCode(apiUrl, clientId, code, codeVerifier, redirectUri);
...
export function getAuthStatus(): { loggedIn: boolean; email?: string; teamName?: string } {
