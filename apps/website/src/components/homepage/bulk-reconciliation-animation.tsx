...
export function BulkReconciliationAnimation({
  ...
  const [transactions, setTransactions] = useState<Transaction[]>(
    initialTransactions.map((t) => ({ ...t, status: 'none' as const, checked: false })),
  )
  ...
  useEffect(() => {
    ...
    shuffledIndices.forEach((index, checkOrder) => {
      ...
      }, allInReviewDoneTime + 200 + randomDelay + checkOrder * 50) // Base delay + random + slight stagger
  ...
  return (
    <div className="w-full h-full flex flex-col relative overflow-hidden border-b border-border">
      {/* Header */}
      ...
      {/* Table */}
      {showTable && (
        <div className="flex-1 min-h-0 overflow-visible border border-border border-b border-border bg-background relative z-0 mt-2 md:mt-3 flex flex-col">
                    ...
                    <div className="flex items-center justify-center h-full">
                      <AnimatePresence mode="wait">
                        {transaction.checked ? (
                          ...
                          >
                            <MaterialIcon name="check" className="text-primary-foreground" size={10} />
                          </motion.div>
                        ) : (
                          <motion.div
                            key="unchecked"
          ...
          {showActionBar && selectedCount > 0 && (
            ...
            >
              ...
              <button className="flex items-center gap-1 px-2 md:px-3 py-1 md:py-1.5 bg-primary text-primary-foreground hover:bg-primary/90 transition-colors">
