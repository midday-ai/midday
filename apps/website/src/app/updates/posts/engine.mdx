...
<br />
The API uses OpenAPI v3.0 and exposes endpoints for transactions, accounts,
search, and balances.
...
<br />
In the end, we have one interface for our customers to search for their
institution. To support this, we retrieve all institutions from our providers,
transform the information, and store it in Typesense. We use a simple Cron job
in Cloudflare every 24 hours to ensure we have the most recent information.

<br />
To connect to Teller, you need to use mTLS to authenticate. Thanks to
Cloudflare, we can just upload our certificate and configure our worker to use
it when needed.
...
<br />
Midday is fully open source. Feel free to check out our code
[here](https://git.new/midday).
