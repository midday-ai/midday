...
const BenefitItem: React.FC<{ icon: React.ElementType; text: string }> = ({
  icon: Icon,
  text,
}) => (
  ...
  return (
    <div className="max-w-2xl mx-auto my-8 p-6 text-white rounded-2xl shadow-md">
      <h2 className="text-2xl font-bold text-white mb-6 text-center">
        For 1 Week, We'll Give You
      </h2>
      <Accordion type="single" collapsible className="mb-6 md:w-[500px]">
        <AccordionItem value="item-1">
          <AccordionTrigger>PDF Guide</AccordionTrigger>
          <AccordionContent>
            PDF Guide offering best practices for invoicing, expense tracking,
            cashflow, cost management, and more
          </AccordionContent>
        </AccordionItem>
        <AccordionItem value="item-2">
          <AccordionTrigger>
            Comprehensive Business Health Audit
          </AccordionTrigger>
          <AccordionContent className="flex text-white justify-start items-start">
            Free in-depth Business Health Audit to outline financial leaks in
        ...
        <AccordionItem value="item-3">
          ...
          <AccordionContent className="flex text-white justify-start items-start">
            Free personalized checklist based on your business's critical needs
