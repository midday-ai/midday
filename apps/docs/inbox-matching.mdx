...
**Flow**: `upsert-transactions` → `embed-transaction` → `match-pending-inbox` → `calculate-suggestions`
...
**Flow**: `process-attachment` → `embed-inbox` → `calculate-suggestions`
...
| `calculate-suggestions` | Performs matching algorithm and creates suggestions | Low |
