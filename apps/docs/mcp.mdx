---
title: "MCP Server"
description: "Connect AI assistants to Abacus using the Model Context Protocol"
---

# MCP Server

Abacus exposes a [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) server that allows AI assistants like Claude, OpenAI, and Cursor to interact with your financial data.

## Endpoint

```
POST https://api.abacuslabs.co/mcp
```

## Authentication

The MCP endpoint requires authentication using your Abacus API key. Include it in the `Authorization` header:
...
1. Go to **Settings → Developer** in the Abacus dashboard
...
|--------------|-------------|
| `abacus://team/info` | Current team information including name and base currency |
| `abacus://categories` | List of all transaction categories with hierarchy and colors |
| `abacus://tags` | List of custom tags used for organizing data |
...
- `openWorldHint: false` - All tools operate only on your Abacus data
