...
<br />
In this blog post where gonna share a little more in details how we implemented
this functionality.

<br />
We have some features like Time Tracker, Reports and files from Vault that our
users shares outside their company and with that we have an authorization layer
on our side using Supabase, but these links are often really long because they
include a unique token.
<br />
...
<br />
![Solomon AI - Overview](/images/overview.png)

<br />
If you look closely you can se our link looks like this:
[https://go.solomon-ai.app/5eYKrmV](https://go.solomon-ai.app/5eYKrmV)

<br />
When the user clicks `Share` we execute a server action using the library
`next-safe-action`
...
<br />
We save it in Supabase and get a id back that we use to generate our sharable
URL.

```typescript
const dub = new Dub({ projectSlug: "solomon-ai" });
...
<br />
With the combination of server actions, Supabase and Dub we can create really
beautiful URLs with analytics on top.

<br />
You can find the source code for this in our repository
[here](https://github.com/SolomonAIEngineering/orbitkit).
